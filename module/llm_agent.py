...
class AIAgent:
    def __init__(
            ...
            max_tokens: int = 1000,
            embedding_system: ChromaEmbeddings = None,
            text_summarizer: ArticleSummarizer = None,
        ...
        if not self.embedding_system and ChromaEmbeddings:
            self.embedding_system = ChromaEmbeddings(
        ...
        # Set up summarizer system
        self.summarizer = text_summarizer
        if not self.summarizer and ArticleSummarizer:
    ...
    def setup_rag_chain(self):
        ...
        try:
            retriever = self.embedding_system.vectorstore.as_retriever(
                search_kwargs={'k': 5}  # Retrieve top k most relevant documents
            )
            ...
            summarize_chain = load_summarize_chain(self.summarizer,
            ...
            self.rag_chain = create_retrieval_chain(
                history_aware_retriever, summarize_chain, question_answer_chain
            )
...
def main():
    ...
    result = Agent.chat('Harga Bitcoin menurut berita terbaru?')
